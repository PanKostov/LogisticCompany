...
const RANDOM_BYTES = [0x43, 0x02, 0xbb, 0xc1, 0xac, 0xf7, 0xb5, 0x8f, 0x8c, 0x83, 0x55, 0x5a, 0x98, 0x23, 0x82, 0x8f]
...
export class Encryptor {
  ...
  async encryptText(text: string): Promise<string> {
    ...
    const textEncrypted = Buffer.concat([cipher.update(text), cipher.final()]).toString('base64')
  ...
  async decryptText(encryptedText: string): Promise<string> {
    ...
    const decryptedText = Buffer.concat([decipher.update(Buffer.from(encryptedText, 'base64')), decipher.final()]).toString()

    return decryptedText
  }

  private async generateKey(): Promise<Buffer> {
    this.key = (await promisify(scrypt)(this.passwordKey, 'salt', 32)) as Buffer
    return this.key
  }
}
